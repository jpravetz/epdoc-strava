...
Kml.prototype.header = function (file) {
    ...
    self.write('  <kml xmlns="http://www.opengis.net/kml/2.2" xmlns:gx="http://www.google.com/kml/ext/2.2" xmlns:kml="http://www.opengis.net/kml/2.2" xmlns:atom="http://www.w3.org/2005/Atom">');
    self.write('  <Document>\n');
    self.write('    <name>Strava Activities</name>\n');
    self.write('    <open>1</open>\n');
    _u.each(self.lineStyles, function (style, name) {
        addLineStyle(name, style);
    });

    function addLineStyle(name, style) {
        self.write('    <Style id="StravaLineStyle' + name + '">\n');
        self.write('      <LineStyle><color>' + style.color + '</color><width>' + style.width + '</width></LineStyle>\n');
        self.write('      <PolyStyle><color>' + style.color + '</color></PolyStyle>\n');
        self.write('    </Style>\n');
    };
...
Kml.prototype.placemark = function( indent, params ) {
    ...
    _u.each(params.coordinates, function (coord) {
        self.write( [coord[1],coord[0],0].join(',') + " " );
    });
    self.writeln( indent+2, '</coordinates>');
    self.writeln( indent+1, '</LineString>');
