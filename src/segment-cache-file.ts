...
import fs from 'fs';
import { SegmentName } from './joins/segment-base';
import { FilePath, IsoDateString, LogFunctions, LogOpts, Metres } from './types';
import { readJson, writeJson } from './util';
import { SegmentSummary } from './joins';
...
export class SegmentCacheFile {
   ...
   */
  constructor(filepath: FilePath, stravaApi: any, opts: LogOpts) {
  ...
  private async getFromServer(): Promise<void> {
    ...
    return this._api
      ...
      .then((resp) => {
        const summarySegments: SegmentSummary[] = isArray(resp) ? resp : [];
        // this._segments = resp;
