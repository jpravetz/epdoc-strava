import fs from 'fs';
import { readJson, Dict } from './util/file';
...
module.exports = function(options) {
  ...
  var self = {
      ...
      var local = {
        append: function(activities) {
          _.each(activities, function(activity) {
            ...
            if (
              (!options.commuteOnly && !options.nonCommuteOnly) ||
              (options.commuteOnly && activity.commute) ||
              (options.nonCommuteOnly && !activity.commute)
            ) {
        ...
        addCoordinates: function(objItem, callback) {
          self.strava.getStream(type, objItem.id, ['latlng'], {}, function(err, data) {
            if (err) {
              ...
              console.log(
                '  Processing coordinates for ' +
                  (type === 'activities' ? objItem.start_date_local + ' ' : '') +
                  objItem.name
              );
              objItem.coordinates = [];
              _.each(data, function(item) {
    ...
    listActivities: function(callback) {
      ...
      console.log(
        'Total distance %s km, elevation gain %s m',
    ...
    writeSegmentsFile: function(segmentsFiles, callback) {
      if (self.segmentsDirty === true) {
        ...
        fs.createReadStream(segmentsFile).pipe(
          fs.createWriteStream(segmentsFile + '_' + dateutil.toFileString(self.segmentsFileLastModified))
        );
